...
| `github.credential_mode` | `github-cli` uses the credential `gh` holds. `none` is an explicit refusal to authorize forge writes, not anonymous access: no pull request is published and no feedback can be admitted. | required |
