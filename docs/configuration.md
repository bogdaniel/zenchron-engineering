...
| `github.credential_mode` | `github-cli` uses your own `gh` login; `github-app` publishes as a GitHub App installation, a machine identity that is not a second human account; `token` publishes as a dedicated runtime account whose token you hold; `none` refuses forge writes. The last three give the runtime a publication identity of its own so your reviews are admissible feedback. | required |
...
| `github.app_id` | The App's numeric id, from its General page. Required by and only used with `credential_mode: "github-app"`. Not a secret. | none |
