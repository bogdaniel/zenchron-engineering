...
| `github.credential_mode` is `none`, or the adapter cannot resolve permissions | the observation says `the configured forge adapter cannot resolve actor permissions` | give the runtime a publication identity that can read the repository: `github-app` with `app_id`, `installation_id` and `private_key_path`, or `token` with `token_path`. Do not go back to `github-cli` — it publishes as you, and then your own reviews are the case below |
...
| The comment is YOURS, and the runtime publishes as you | reason `authored by this runtime` for your own login, and `autonomy doctor` warns that `publication identity` and `human feedback actor` are the SAME account | give the runtime an identity of its own — `github.credential_mode: "github-app"` is the machine one and needs no second GitHub account. See the provisioning runbook in [github-feedback.md](github-feedback.md). The guard is not the problem and must not be relaxed |
