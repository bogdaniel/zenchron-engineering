...
type OperationStatus struct {
	...
	// Invocations is how many times a provider has actually been invoked for
	// this operation. It is reported alongside Attempt because the two
	// legitimately disagree: a provider condition that routes to an external
	// wait refunds the attempt and does not unhappen the invocation. Without
	// it, an operator reading "attempt 1/2" after two provider calls has no
	// way to tell a refunded wait from a stuck scheduler.
...
func (r *EngineeringRuntime) Status(runID string) (StatusReport, error) {
	...
	if op, ok := state.currentOperation(); ok {
		status := OperationStatus{
			...
			Attempt: op.Attempt, MaxAttempts: op.MaxAttempts, Invocations: op.Invocations,
